...
def z_decode_int(buf, length):
  value = z_decode_uint(buf, length)
  return z_decode_zigzag(value)

def z_decode_bytes(buf, length):
  return buf[:length]

def z_decode_string(buf, length):
  return str(z_decode_bytes(buf, length))
...
def z_encode_field_int(field_id, value):
  return z_encode_field_uint(field_id, z_encode_zigzag(value))

def z_encode_field_bytes(field_id, value):
  return z_encode_field(field_id, len(value)) + value
