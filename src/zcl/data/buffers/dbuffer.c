...
#define Z_DBUF_NODE_MIN       (118)
#define Z_DBUF_NODE_MAX       (246)
                                              ...
                                              size_t size)
{
  ...
  /* Allocate new node */
  size = (size > Z_DBUF_NODE_MIN) ? Z_DBUF_NODE_MAX : Z_DBUF_NODE_MIN;
  node = z_memory_alloc(self->memory, z_dbuf_node_t, sizeof(z_dbuf_node_t) - 6 + size);
  if (Z_MALLOC_IS_NULL(node))
    return(NULL);

  node->next = NULL;
  node->size = (size & 0xff) >> 1;
  node->alloc = 1;
...
int z_dbuf_writer_add (z_dbuf_writer_t *self, const void *data, size_t size) {
  ...
  while (size > 0) {
    ...
    if (self->whead != Z_DBUF_NO_HEAD) {
      ...
      self->whead = (node->data - pbuf) & 0xff;
      --(self->avail);
      to_add = z_min(size, self->avail);
      *pbuf++ = to_add;
    }
...
static z_dbuf_node_t *__dbuf_reader_remove_head (z_dbuf_reader_t *self) {
  ...
  if (node->alloc) {
    z_memory_free(self->memory, node);
  }
  return(self->head);
}
