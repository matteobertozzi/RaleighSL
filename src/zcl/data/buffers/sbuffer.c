...
static uint16_t __dkey_dvalue_replace (uint8_t *sbuffer, uint16_t pos, const z_memslice_t *key, const z_memslice_t *value) {
  ...
  uint16_t vsize;
  pbuf += *((const uint16_t *)pbuf) + 2;
  vsize = *((const uint16_t *)pbuf);
  if (value->size > vsize) {
...
static void __dkey_dvalue_lookup (const uint8_t *sbuffer, uint16_t pos, z_memslice_t *value) {
  ...
  value->size = *((const uint16_t *)pbuf);
  value->data = pbuf + 2;
}
...
static void __dkey_dvalue_get (const uint8_t *sbuffer, uint16_t pos, z_memslice_t *key, z_memslice_t *value) {
  ...
  key->size = *((uint16_t *)pbuf);
  key->data = pbuf + 2;
  pbuf += 2 + key->size;

  value->size = *((uint16_t *)pbuf);
  value->data = pbuf + 2;
}
...
static void __dkey_fvalue_lookup (const uint8_t *sbuffer, uint16_t pos, z_memslice_t *value) {
  ...
  value->size = __SBUFFER_HEAD(sbuffer)->vmax;
  value->data = pbuf + 2;
}
...
static void __fkey_fvalue_lookup (const uint8_t *sbuffer, uint16_t pos, z_memslice_t *value) {
  const uint8_t *pbuf = sbuffer + pos;
  value->size = __SBUFFER_HEAD(sbuffer)->vmax;
  value->data = pbuf + __SBUFFER_HEAD(sbuffer)->kmax;
}

static void __fkey_fvalue_get (const uint8_t *sbuffer, uint16_t pos, z_memslice_t *key, z_memslice_t *value) {
  const uint8_t *pbuf = sbuffer + pos;

  key->size = __SBUFFER_HEAD(sbuffer)->kmax;
  key->data = pbuf;

  value->size = __SBUFFER_HEAD(sbuffer)->vmax;
  value->data = pbuf + key->size;
}
...
/* ============================================================================
 *  PRIVATE SBuffer Map Vector Iterator methods
 */
static int __sbuffer_iter_begin (void *self) {
  z_sbuffer_iter_t *iter = (z_sbuffer_iter_t *)self;
  if (Z_LIKELY(__SBUFFER_HEAD(iter->sbuffer)->nitems > 0)) {
    z_map_entry_t *entry = &(iter->ihead.entry_buf);
    z_sbuffer_get(iter->sbuffer, 0, &(entry->key), &(entry->value));
...
static int __sbuffer_iter_next (void *self) {
  z_sbuffer_iter_t *iter = (z_sbuffer_iter_t *)self;
  if (Z_LIKELY(iter->current < __SBUFFER_HEAD(iter->sbuffer)->nitems)) {
    z_map_entry_t *entry = &(iter->ihead.entry_buf);
    z_sbuffer_get(iter->sbuffer, iter->current, &(entry->key), &(entry->value));
    iter->ihead.entry = entry;
    ++iter->current;
    return(1);
  }
...
static int __sbuffer_iter_seek (void *self, const z_memslice_t *key) {
  z_sbuffer_iter_t *iter = (z_sbuffer_iter_t *)self;
  iter->ihead.entry = NULL;
  return(-1);
}

static const z_vtable_map_iterator_t __sbuffer_map_iterator = {
  .begin = __sbuffer_iter_begin,
  .next  = __sbuffer_iter_next,
  .seek  = __sbuffer_iter_seek,
};

/* ============================================================================
 *  PUBLIC Map-Entry Vector Iterator methods
 */
void z_sbuffer_iter_open (z_sbuffer_iter_t *self, const uint8_t *sbuffer) {
  z_map_iterator_init(&(self->ihead), &__sbuffer_map_iterator);
