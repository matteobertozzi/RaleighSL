...
static size_t __ringbuf_read_iov (z_ringbuf_t *self, struct iovec iov[2]) {
  ...
  iov[0].iov_base = self->buffer + head;
  iov[0].iov_len  = z_ringbuf_size(self) - head;
  iov[1].iov_base = self->buffer;
  iov[1].iov_len  = tail;
 ...
 */
int z_ringbuf_alloc (z_ringbuf_t *self, uint32_t size) {
  uint8_t *buffer;
  size = z_align_up(size, 8);
  buffer = z_memory_alloc(z_global_memory(), uint8_t, size);
  if (Z_MALLOC_IS_NULL(buffer))
    return(1);
  z_ringbuf_init(self, buffer, size);
  self->siz |= 1;
  return(0);
}
...
void z_ringbuf_init (z_ringbuf_t *self, uint8_t *buffer, uint32_t size) {
  Z_ASSERT(z_is_pow2(size), "Expected power of 2 size %u", size);
  self->siz = size;
  self->head = 0;
  self->tail = 0;
  self->buffer = buffer;
...
ssize_t z_ringbuf_fd_dump (z_ringbuf_t *self, int fd) {
  ...
  if (wr > 0) {
    self->head = (self->head + wr) & ((z_ringbuf_size(self) << 1) - 1);
  }
  return(wr);
}
...
size_t z_ringbuf_rbuffer (z_ringbuf_t *self, const uint8_t **buffer) {
  ...
  *buffer = self->buffer + head;
  n = (head < tail) ? (tail - head) : (z_ringbuf_size(self) - head);

  self->head = (self->head + n) & ((z_ringbuf_size(self) << 1) - 1);
  return(n);
}
...
size_t z_ringbuf_pop (z_ringbuf_t *self, void *buf, size_t n) {
  ...
  self->head = (self->head + total) & ((z_ringbuf_size(self) << 1) - 1);
  return(total);
}
...
size_t z_ringbuf_used (const z_ringbuf_t *self) {
  ...
  size_t head = self->head & mask;
  size_t tail = self->tail & mask;
  return((head <= tail) ? (tail - head) : ((z_ringbuf_size(self) - head) + tail));
}
