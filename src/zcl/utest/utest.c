                   ...
                   z_utest_func_t func, int timeout_msec)
{
  ...
  if (env.perf_bytes > 0) {
    z_human_size_d(hbuf, sizeof(hbuf), env.perf_bytes / z_timer_secs(&env.timer));
    fprintf(stderr, " - perf-bytes %s/sec", hbuf);

    z_human_ops_d(hbuf, sizeof(hbuf), env.perf_bytes);
    fprintf(stderr, " (data %s)", hbuf);
  }

  if (env.perf_ops > 0) {
    z_human_ops_d(hbuf, sizeof(hbuf), env.perf_ops / z_timer_secs(&env.timer));
    fprintf(stderr, " - perf-ops %s/sec", hbuf);

    z_human_ops_d(hbuf, sizeof(hbuf), env.perf_ops);
