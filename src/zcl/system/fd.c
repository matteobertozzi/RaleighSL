...
int z_read_file (const char *path, void *buffer, size_t length) {
  uint8_t *pbuf = Z_CAST(uint8_t, buffer);
  int fd;

  if (Z_UNLIKELY((fd = open(path, O_RDONLY)) < 0)) {
    Z_LOG_ERRNO_WARN("failed open(O_RDONLY) for %s", path);
    return(-1);
  }

  while (length > 0) {
    ssize_t rd = read(fd, pbuf, length);
    if (rd <= 0) break;
