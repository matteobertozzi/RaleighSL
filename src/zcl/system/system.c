...
int z_system_rand (void *buffer, int length) {
  return z_read_file("/dev/random", buffer, length);
}

int z_system_urand (void *buffer, int length) {
  return z_read_file("/dev/urandom", buffer, length);
}
