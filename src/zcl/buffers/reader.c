                            ...
                            size_t length, int8_t *value)
{
  ...
  *value = z_decode_zigzag32(uvalue);
  return r;
}
                              ...
                              size_t length, uint16_t *value)
{
  ...
  z_decode_uint16(pbuf, length, value);
  return(0);
}
                              ...
                              size_t length, uint32_t *value)
{
  ...
  z_decode_uint32(pbuf, length, value);
  return(0);
}
                             ...
                             size_t length, int64_t *value)
{
  ...
  *value = z_decode_zigzag64(uvalue);
  return r;
}
                              ...
                              size_t length, uint64_t *value)
{
  ...
  z_decode_uint64(pbuf, length, value);
  return(0);
}
                                ...
                                size_t length, z_buffer_t *value)
{
  ...
  z_buffer_ensure(value, length);
  pbuf = z_v_reader_fetch(vtable, self, value->block, length);
  if (Z_UNLIKELY(pbuf == NULL))
    return(1);

  if (pbuf != value->block) {
    z_buffer_append(value, pbuf, length);
  } else {
    value->size += length;
  }
  return(0);
                             ...
                             size_t length, z_bytes_ref_t *value)
{
  ...
  pbuf = z_v_reader_fetch(vtable, self, bytes->slice.data, length);
  if (Z_UNLIKELY(pbuf == NULL)) {
    z_bytes_free(bytes);
    return(1);
  }
