...
static void ripemd160_init (RIPEMD_Context *self) {
    z_memcpy(self->h, initial_h, RIPEMD160_DIGEST_SIZE);
    z_memzero(&self->buf, sizeof(self->buf));
    self->length = 0;
    self->bufpos = 0;
}
...
static void ripemd160_update (RIPEMD_Context *self,
                              const void *data,
                              size_t length)
{
    ...
    while (length > 0) {
        ...
        if (length >= bytes_needed) {
            ...
            z_memcpy(&self->buf.b[self->bufpos], p, bytes_needed);
        ...
        z_memcpy(&self->buf.b[self->bufpos], p, length);
        self->bufpos += length;
...
static void ripemd160_digest (RIPEMD_Context *self, uint8_t hash[20]) {
    /* Append the padding */
...
void z_hash160_ripemd (uint8_t hash[20],
                       ...
                       unsigned int size)
{
    RIPEMD_Context context;

    ripemd160_init(&context);
    ripemd160_update(&context, data, size);
    ripemd160_digest(&context, hash);
}

/* ============================================================================
 *  Hash160 - RIPEMD Plugin
 */
static int __ripemd_init (z_hash_t *hash) {
    RIPEMD_Context *context;

    if ((context = z_object_struct_alloc(hash, RIPEMD_Context)) == NULL)
        return(1);
...
static void __ripemd_uninit (z_hash_t *hash) {
    z_object_struct_free(hash, RIPEMD_Context, hash->plug_data.ptr);
}

static void __ripemd_update (z_hash_t *hash, const void *blob, unsigned int n) {
    RIPEMD_Context *context = (RIPEMD_Context *)hash->plug_data.ptr;
    ripemd160_update(context, blob, n);
}

static void __ripemd_digest (z_hash_t *hash, void *digest) {
    RIPEMD_Context *context = (RIPEMD_Context *)hash->plug_data.ptr;
    ripemd160_digest(context, digest);
}

z_hash_plug_t z_hash160_plug_ripemd = {
    .init   = __ripemd_init,
    .uninit = __ripemd_uninit,
    .update = __ripemd_update,
    .digest = __ripemd_digest,
};
