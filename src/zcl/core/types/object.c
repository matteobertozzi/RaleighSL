                        ...
                        z_memory_t *memory, ...)
{
    ...
    if (object != NULL) {
        ...
        object = z_memory_alloc(memory, z_object_t, type->size);
        if (Z_UNLIKELY(object == NULL)) return(NULL);
    ...
    object->flags.uflags32 = 0;

    /* Call the object constructor */
    va_start(args, memory);
    if (type->ctor(object, memory, args)) {
