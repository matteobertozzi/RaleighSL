...
int z_semaphore_try_acquire_timed (z_semaphore_t *self, int n, unsigned int usec) {
  uint64_t st, elapsed = 0;
  int acquired = 0;
  z_mutex_lock(&(self->mutex));
  st = z_time_micros();
  while (n > self->available && elapsed < usec) {
    z_wait_cond_wait(&(self->wcond), &(self->mutex), usec);
    elapsed = z_time_micros() - st;
  }
  if (n <= self->available) {
    self->available -= n;
    acquired = 1;
