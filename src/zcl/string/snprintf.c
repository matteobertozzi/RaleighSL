...
#if 0
int z_vfprintf (FILE *stream, const char *frmt, va_list ap) {
  char buffer[8192];
  z_vsvnprintf(buffer, sizeof(buffer), frmt, ap);
  return fprintf(stream, "%s", buffer);
}

int z_fprintf (FILE *stream, const char *frmt, ...) {
  va_list ap;
  int n;

  va_start(ap, frmt);
  n = z_vfprintf(stream, frmt, ap);
  va_end(ap);
