...
static int __epoll_get_timer_spec (struct itimerspec *ts, uint32_t msec) {
  struct timespec now;
  uint64_t ts_timeout;
  uint64_t ts_start;

  if (Z_UNLIKELY(clock_gettime(CLOCK_REALTIME, &now) < 0)) {
    Z_LOG_ERRNO_WARN("clock_gettime(CLOCK_REALTIME)");
    return(-1);
  }

  ts_timeout = Z_TIME_MILLIS_TO_NANOS(msec);
  ts_start = (Z_TIME_SECONDS_TO_NANOS(now.tv_sec) + now.tv_nsec) + ts_timeout;

  ts->it_interval.tv_sec  = (ts_timeout / 1000000000ul);
  ts->it_interval.tv_nsec = (ts_timeout % 1000000000ul);
  ts->it_value.tv_sec  = (ts_start / 1000000000ul);
  ts->it_value.tv_nsec = (ts_start % 1000000000ul);
  return(0);
}
                          ...
                          uint32_t msec)
{
  if (msec > 0) {
    ...
    if (Z_UNLIKELY(__epoll_get_timer_spec(&ts, msec) < 0))
      return(-1);

    if (Z_UNLIKELY(timerfd_settime(entity->fd, TFD_TIMER_ABSTIME, &ts, NULL) < 0)) {
      Z_LOG_ERRNO_WARN("timerfd_settime(fd=%d msec=%"PRIu32"msec)", entity->fd, msec);
      return(-1);
    }
