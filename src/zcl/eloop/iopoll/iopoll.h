...
struct z_iopoll_stats {
  ...
  uint64_t        iowait_events[21];

  z_histogram_t   ioread;
  uint64_t        ioread_events[21];

  z_histogram_t   iowrite;
  uint64_t        iowrite_events[21];

  z_histogram_t   event;
  uint64_t        event_events[21];

  z_histogram_t   timeout;
  uint64_t        timeout_events[21];
};

struct z_iopoll_engine {
  z_opaque_t       data;
  z_iopoll_load_t  load;
...
#define z_iopoll_stats_add_active(self, nsec)                      \
  (self)->load.active[(self)->load.tail] += (nsec)
