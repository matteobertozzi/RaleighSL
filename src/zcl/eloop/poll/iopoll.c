...
#define __time_u32_usec(nsec)     ((uint32_t)(((nsec) / 1000) / 0xffffffff))
...
#define __write_time_elapsed(entity, now)                                 \
 ((__time_u32_usec(now) - (entity)->last_write_ts) > 1000000ull)
  ...
  if (has_data || events & Z_IOPOLL_WRITABLE) {
    if (Z_LIKELY(has_data)) {
      ...
      Z_LOG_DEBUG("switch state %u %u diff %u (too many write events)",
                  __time_u32_usec(now), entity->last_write_ts,
                  __time_u32_usec(now) - entity->last_write_ts);
...
void z_iopoll_engine_dump (z_iopoll_engine_t *engine, FILE *stream) {
  ...
  __iopoll_load_dump(&(stats->ioload), stream);
  fprintf(stream, "\n");
...
void z_iopoll_set_writable (z_iopoll_engine_t *engine,
                            ...
                            int writable)
{
  entity->last_write_ts = __time_u32_usec(z_time_monotonic());
...
static ssize_t __iopoll_entity_readv (void *udata, const struct iovec *iov, int iovcnt) {
  return z_fd_readv(Z_IOPOLL_ENTITY_FD(udata), iov, iovcnt);
}

static ssize_t __iopoll_entity_writev (void *udata, const struct iovec *iov, int iovcnt) {
  return z_fd_writev(Z_IOPOLL_ENTITY_FD(udata), iov, iovcnt);
}
