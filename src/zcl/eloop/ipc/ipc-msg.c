...
static const uint64_t __STATS_TIME_HISTO_BOUNDS[21] = {
  Z_TIME_USEC(5),   Z_TIME_USEC(10),  Z_TIME_USEC(25),  Z_TIME_USEC(50),
  Z_TIME_USEC(75),  Z_TIME_USEC(100), Z_TIME_USEC(150), Z_TIME_USEC(200),
  Z_TIME_USEC(350), Z_TIME_MSEC(500), Z_TIME_USEC(750), Z_TIME_MSEC(5),
  Z_TIME_MSEC(10),  Z_TIME_MSEC(25),  Z_TIME_MSEC(75),  Z_TIME_MSEC(100),
  Z_TIME_MSEC(250), Z_TIME_MSEC(500), Z_TIME_MSEC(750), Z_TIME_SEC(1),
  0xffffffffffffffffll,
};

static const uint64_t __STATS_SIZE_HISTO_BOUNDS[25] = {
    16,  32,  48,  64,  80,  96, 112, 128, 256, 512,
  Z_KB(1),   Z_KB(2),   Z_KB(4),   Z_KB(8),   Z_KB(16),
  Z_KB(32),  Z_KB(64),  Z_KB(128), Z_KB(256), Z_KB(512),
  Z_MB(1),   Z_MB(8),   Z_MB(16),  Z_MB(32),  0xffffffffffffffffll,
};
...
void z_ipc_msg_stats_dump (z_ipc_msg_stats_t *self,
                           ...
                           FILE *stream)
{
  printf("IPC-MSG %s Stats\n", msg_type);
  printf("==================================\n");

  fprintf(stream, "%s Latency", msg_type);
  z_histogram_dump(&(self->latency), stream, z_human_dtime);
  fprintf(stream, "\n");

  fprintf(stream, "%s Fwd Size", msg_type);
  z_histogram_dump(&(self->fwd_size), stream, z_human_dsize);
  fprintf(stream, "\n");

  fprintf(stream, "%s Body Size", msg_type);
  z_histogram_dump(&(self->body_size), stream, z_human_dsize);
  fprintf(stream, "\n");

  fprintf(stream, "%s Data Size", msg_type);
  z_histogram_dump(&(self->data_size), stream, z_human_dsize);
  fprintf(stream, "\n");
}
