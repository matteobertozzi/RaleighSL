...
struct avl_remove_data {
  z_tree_node_t *stack[__DSTACK_SIZE];     /* Nodes */
                         ...
                         z_tree_node_t *node)
{
  ...
  for (p = v->parent, k = 0; p != node; p = p->child[v->dstack[k]], ++k) {
    if (v->dstack[k] == 0)
      --(p->balance);
    else
      ++(p->balance);
...
static z_tree_node_t *__avl_detach_floor (const z_tree_info_t *tree,
                                          z_tree_node_t **root,
                                          const void *key,
                                          void *udata)
{
  ...
  p = p->child[0];
  while (p != NULL) {
    int cmp = tree->key_compare(udata, p, key);
    if (cmp < 0) {
      if (p->child[1] != NULL) {
        v.dstack[v.istack] = cmp < 0;
        v.stack[v.istack++] = p;
        p = p->child[1];
      } else {
        if (cmp < 0)
...
const z_tree_plug_t z_tree_avl = {
  ...
  .detach_floor = __avl_detach_floor,
};
