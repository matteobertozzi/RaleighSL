...
struct avl_insert_data {
  uint8_t da[Z_TREE_MAX_HEIGHT]; /* Cached comparison results. */
...
struct avl_remove_data {
  ...
  uint8_t da[Z_TREE_MAX_HEIGHT];         /* Directions moved from stack nodes */
  int k;                                 /* Stack pointer. */
};
                         ...
                         void *data)
{
  ...
  if ((*p = z_memory_struct_alloc(z_global_memory(), z_tree_node_t)) == NULL)
    return(-1);
                         ...
                         const void *key)
{
  ...
  if (tree->data_free != NULL)
    tree->data_free(tree->user_data, p->data);
  z_memory_struct_free(z_global_memory(), z_tree_node_t, p);
