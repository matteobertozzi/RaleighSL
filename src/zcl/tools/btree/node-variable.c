                              ...
                              z_btree_item_t *item)
{
  ...
  z_decode_uint32(pkey, length[2], &(item->vsize));   pkey += length[2];
...
static uint32_t __node_calc_khead (const z_btree_item_t *item, uint8_t head[3]) {
  head[0] = (item->kprefix > 0) ? z_uint32_size(item->kprefix) : 0;
  head[1] = z_uint32_size(item->ksize);
  head[2] = z_uint32_size(item->vsize);
...
static int __node_has_space (const uint8_t *node, const z_btree_item_t *item) {
  uint8_t length[3];
  size_t required;
  required = __node_calc_khead(item, length) + item->ksize + item->vsize;
...
static int __node_append (uint8_t *node, const z_btree_item_t *item) {
  ...
  *pkey++ = (length[0] << 5) | ((length[1] - 1) << 3) | ((length[2] - 1) << 1);
  ...
  z_encode_uint(pkey, length[2], item->vsize);   pkey += length[2];
...
const z_vtable_btree_node_t z_btree_vnode = {
