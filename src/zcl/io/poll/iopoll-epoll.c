...
#define __EPOLL_QSIZE           (512)
#define __EPOLL_TIMEOUT_MSEC    (2000)

static int __epoll_open (z_iopoll_engine_t *engine) {
  if ((engine->data.fd = epoll_create(__EPOLL_QSIZE)) < 0) {
    Z_LOG_ERRNO_ERROR("epoll_create()");
    return(-1);
  }
  return(0);
}

static void __epoll_close (z_iopoll_engine_t *engine) {
  close(engine->data.fd);
}

static int __epoll_insert (z_iopoll_engine_t *engine,
                           z_iopoll_entity_t *entity)
{
  ...
  if (Z_LIKELY(entity->iflags8 & Z_IOPOLL_READABLE))
  ...
  if (Z_UNLIKELY(epoll_ctl(engine->data.fd, op, entity->fd, &event) < 0)) {
...
static int __epoll_remove (z_iopoll_engine_t *engine,
                           z_iopoll_entity_t *entity)
{
  struct epoll_event event;
  ...
  event.events = 0;
  event.data.ptr = entity;
  if (Z_UNLIKELY(epoll_ctl(engine->data.fd, EPOLL_CTL_DEL, entity->fd, &event) < 0)) {
...
static void __epoll_poll (z_iopoll_engine_t *engine) {
  ...
  while (*is_running) {
    ...
    z_timer_start(&timer);
    n = epoll_wait(engine->data.fd, events, __EPOLL_QSIZE, __EPOLL_TIMEOUT_MSEC);
    if (Z_UNLIKELY(n < 0)) {
    ...
    for (e = events; n--; ++e) {
      ...
      z_iopoll_process(engine, Z_IOPOLL_ENTITY(e->data.ptr), eflags);
    }
  }
}
