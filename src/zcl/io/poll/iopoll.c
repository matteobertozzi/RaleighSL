...
struct z_iopoll {
  ...
  const uint8_t *is_running;
  uint8_t pad1[Z_CACHELINE_PAD_FIELDS(z_vtable_iopoll_t *, int *)];
 ...
 */
static const z_vtable_iopoll_entity_t *__iopoll_entity_vtables[] = {
  /*  0- 7 */ NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
  /*  8-15 */ NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
};

static uint8_t __iopoll_entity_register (const z_vtable_iopoll_entity_t *vtable) {
  uint8_t i;
...
#define __iopoll_engine_insert(engine, entity)                      \
  __global_iopoll->vtable->insert(engine, entity)
...
int z_iopoll_open (unsigned int ncores) {
  ...
  __global_iopoll = z_allocator_alloc(z_global_allocator(), z_iopoll_t, mmsize);
  if (__global_iopoll == NULL) {
  ...
  if (z_thread_pool_open(&(__global_iopoll->thread_pool), ncores)) {
    ...
    z_allocator_free(z_global_allocator(), __global_iopoll);
...
int z_iopoll_add (z_iopoll_entity_t *entity) {
  ...
  return(__iopoll_engine_insert(&(__global_iopoll->engines[entity->ioengine]), entity));
}
...
int z_iopoll_poll (int detached, const uint8_t *is_running) {
  __global_iopoll->is_running = (is_running ? is_running : z_global_is_running());
...
#define __write_time_elapsed(entity, now)                    \
  ((((now) / 1000000u) - (entity)->last_write_ts) > 0)

void z_iopoll_process (z_iopoll_engine_t *engine,
                       z_iopoll_entity_t *entity,
                       uint32_t events)
{
  const z_vtable_iopoll_entity_t *vtable = __iopoll_entity_vtables[entity->type];
  uint64_t stime, now;
  ...
  if (events & Z_IOPOLL_WRITABLE || entity->iflags8 & Z_IOPOLL_HAS_DATA) {
    if (entity->iflags8 & Z_IOPOLL_HAS_DATA) {
      ...
      if (!(entity->iflags8 & Z_IOPOLL_HAS_DATA)) {
        ...
        __iopoll_engine_insert(engine, entity);
      }
...
void z_iopoll_set_writable (z_iopoll_entity_t *entity, int writable) {
  ...
  entity->last_write_ts = z_time_secs();
  z_ticket_release(&(entity->lock));
}
