                    ...
                    uint64_t length)
{
    ...
    if (length <= 8) {
        ...
        /* External Length */
        unsigned int flen = z_uint64_bytes(length);
        buf[0] = (1 << 7) | (((flen - 1) & 0x7) << 4);
        z_encode_uint(buf + 1, flen, length);
        elength += flen;
    }
    ...
    if (field_id <= 13) {
        ...
        unsigned int flen = z_uint32_bytes(field_id);
        buf[0] |= (flen - 1);
        z_encode_uint(buf + elength, flen, field_id);
        elength += flen;
    }
                    ...
                    uint64_t *length)
{
    ...
    if (buf[0] & (1 << 7)) {
        /* External Length */
        elength += flen;
        if (buflen < elength) return(-1);
        z_decode_uint64(buf + 1, flen, length);
    } else {
        /* Internal Length (uint8-uint64) */
        *length = flen;
    }
    ...
    if (flen >= 2) {
        ...
        *field_id = flen - 2;
        ...
        flen += 1;
        if (buflen < (elength + flen)) return(-1);
        z_decode_uint16(buf + elength, flen, field_id);
        elength += flen;
    }
