 ...
 */
static int z_decode_4int_head (unsigned int head[4], unsigned char hbuf) {
...
{
    unsigned int a_length = z_uint32_bytes(a);
    unsigned int b_length = z_uint32_bytes(b);
    unsigned int c_length = z_uint32_bytes(c);
    unsigned int d_length = z_uint32_bytes(d);
 ...
 */
static int z_decode_3int_head (unsigned int head[3], unsigned char hbuf) {
...
int z_encode_3int (unsigned char *buf, uint64_t a, uint64_t b, uint32_t c) {
    unsigned int a_length = z_uint64_bytes(a);
    unsigned int b_length = z_uint64_bytes(b);
    unsigned int c_length = z_uint32_bytes(c);
    unsigned char *pbuf = buf + 1;
