...
static int __msgbuf_parse_head (z_ringbuf_t *ringbuf, uint8_t *version, uint32_t *size) {
  ...
  if ((pbuf = (uint8_t *)z_ringbuf_fetch(ringbuf, &buf, 8)) == NULL) {
    return(0);
  }
  ...
  *size    = pbuf[1] | pbuf[2] << 8 | pbuf[3] << 16;
  ...
  if (Z_UNLIKELY(*version > Z_MSGBUF_VERSION ||
                 ...
                 (8 + *size) > ringbuf->size))
  {
    Z_LOG_FATAL("Invalid ipc-message header: magic %x version %u size %u",
                magic, *version, *size);
    return(-2);
  }
...
int z_ipc_msgbuf_flush (z_ipc_msgbuf_t *self, z_ipc_client_t *client) {
  if (__ipc_outbuf_flush(self, Z_IOPOLL_ENTITY_FD(client)))
    return(-1);
  z_ipc_client_set_writable(client, self->obuffer.m_count > 0);
  return(0);
}
