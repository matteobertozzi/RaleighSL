...
static z_ipc_client_t *__ipc_client_alloc (z_ipc_server_t *server, int csock) {
  ...
  if (proto->msg_exec != NULL) {
    z_ipc_msgbuf_open(&(Z_IPC_MSG_CLIENT(client)->msgbuf));
    Z_IPC_MSG_CLIENT(client)->refs = 1;
  }
...
static const z_vtable_iopoll_entity_t __ipc_raw_client_vtable = {
...
static const z_vtable_iopoll_entity_t __ipc_msg_client_vtable = {
