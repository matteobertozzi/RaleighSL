...
int z_field_has_bytes (const uint8_t *buf, size_t length) {
  switch (buf[0] >> 3) {
    case 30: return length >= 2;
    case 31: return length >= 3;
    default: return length >= 1;
  }
}

void z_field_read (uint8_t *buf, uint16_t *field_id, int *vlength) {
  *vlength = 1 + (buf[0] & 7);
  *field_id = buf[0] >> 3;
  switch (*field_id) {
    case 30:
