...
#define __name_to_key(key, name)                                              \
  z_hash256_sha(key, (name)->slice.data, (name)->slice.length)
...
static struct object_entry *__object_entry_alloc (const z_bytes_t *name, uint64_t oid) {
 ...
 */
raleighfs_errno_t __semantic_init (raleighfs_t *fs) {
  z_skip_list_t *table;

  table = z_skip_list_alloc(NULL, __object_entry_compare, __object_entry_free, fs, (unsigned int)fs);
...
raleighfs_errno_t __semantic_load (raleighfs_t *fs) {
  return(RALEIGHFS_ERRNO_NONE);
}

raleighfs_errno_t __semantic_unload (raleighfs_t *fs) {
  z_skip_list_free(__semantic_table(fs));
  return(RALEIGHFS_ERRNO_NONE);
}

raleighfs_errno_t __semantic_commit (raleighfs_t *fs) {
  z_skip_list_commit(__semantic_table(fs));
  return(RALEIGHFS_ERRNO_NONE);
}

raleighfs_errno_t __semantic_rollback (raleighfs_t *fs) {
  z_skip_list_rollback(__semantic_table(fs));
  return(RALEIGHFS_ERRNO_NONE);
}

raleighfs_errno_t __semantic_create (raleighfs_t *fs,
                                     z_bytes_t *name,
                                     uint64_t oid)
{
  struct object_entry *entry;
...
raleighfs_errno_t __semantic_lookup (raleighfs_t *fs,
...
raleighfs_errno_t __semantic_unlink (raleighfs_t *fs,
                                     const z_bytes_t *name,
                                     uint64_t *oid)
{
  struct object_entry *entry;
...
const raleighfs_semantic_plug_t raleighfs_semantic_flat = {
  .info = {
