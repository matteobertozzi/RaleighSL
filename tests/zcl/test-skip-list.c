...
static int __test_setup (z_test_t *test) {
    ...
    z_skip_list_alloc(&(data->skip), &(data->memory),
                       ...
                       const void *value)
{
    ...
    if (z_skip_list_insert(&(data->skip), obj))
        return(2);

    if (z_skip_list_lookup(&(data->skip), &olk) != obj)
        return(3);
...
static int __test_iter_next (z_skip_iter_t *iter,
                             const void *key)
{
    struct object *obj;

    obj = (struct object *)z_skip_iter_next(iter);
    if (obj == NULL)
        return(key != NULL);
...
static int __test_iter_prev (z_skip_iter_t *iter,
                             const void *key)
{
    struct object *obj;

    obj = (struct object *)z_skip_iter_prev(iter);
    if (obj == NULL)
        return(key != NULL);
...
static int __test_iter (z_test_t *test) {
    ...
    z_skip_iter_init(&iter, &(((struct user_data *)test->user_data)->skip));

    obj = (struct object *)z_skip_iter_lookup_min(&iter);
    ...
    obj = (struct object *)z_skip_iter_lookup_max(&iter);
