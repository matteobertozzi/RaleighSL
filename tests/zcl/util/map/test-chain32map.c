...
static void test_perf (uint32_t isize) {
  ...
  max_items = z_chain32_map_init(block, BLOCK, isize, 6);
  Z_DEBUG("============================ PERF block=%u items=%u isize=%u", block, max_items, isize);

  z_test_map_perf(max_items, max_items * 2, &entry, HASHFUNC,
  /* insert */ {
    uint8_t *pkey = z_chain32_map_put(block, entry.hash, CMPFUNC, &(entry.key), NULL);
    memcpy(pkey, &entry, isize);
  },
  /* lookup */ {
    z_chain32_map_get(block, entry.hash, CMPFUNC, &(entry.key), NULL);
  },
  /* remove */ {
    z_chain32_map_remove(block, entry.hash, CMPFUNC, &(entry.key), NULL);
  },
  /* reset */ {
    z_chain32_map_init(block, BLOCK, isize, 8);
  });
...
static void test_validate (uint32_t isize) {
  ...
  max_items = z_chain32_map_init(block, sizeof(block), isize, 8);
  ...
  /* lookup */ {
    z_test_map_entry_t *res = z_chain32_map_get(block, lkpentry.hash, CMPFUNC, &(lkpentry.key), NULL);
  ...
  /* remove */ {
    z_test_map_entry_t *res = z_chain32_map_remove(block, entry.hash, CMPFUNC, &(entry.key), NULL);
    z_test_map_validate_entry("remove", isize, res, &entry);
  },
  /* reset */ {
    z_chain32_map_init(block, sizeof(block), isize, 8);
  });
}
