...
static int __object_node_cmp (void *udata, const z_avl_link_t *a, const z_avl_link_t *b) {
...
static int __object_key_cmp (void *udata, const void *key, const z_avl_link_t *x) {
...
static void test_crud (z_utest_env_t *env) {
  ...
  z_avl_tree_init(&tree);
  for (i = 4; i >= 0; --i) {
    nodes[i].id = i + 1;
    z_avl_tree_insert(&tree, __object_node_cmp, &(nodes[i].link), NULL);
    for (j = 5; j > i; --j) {
      struct object_node *obj;
      node = z_avl_tree_lookup(&tree, __object_key_cmp, &j, NULL);
  ...
  i = 0;
  node = z_avl_tree_lookup(&tree, __object_key_cmp, &i, NULL);
  ...
  for (i = 1; i <= 5; ++i) {
    z_avl_tree_remove(&tree, __object_key_cmp, &i, NULL);
    for (j = 1; j <= i; ++j) {
      node = z_avl_tree_lookup(&tree, __object_key_cmp, &j, NULL);
      z_assert_null(env, node);
    }
...
static void test_iter (z_utest_env_t *env) {
  ...
  z_avl_iter_init(&iter, &tree);
  node = z_avl_iter_seek_begin(&iter);
  for (i = 0; node != NULL; i += 2) {
    struct object_node *obj = z_container_of(node, struct object_node, link);
    z_assert_int_equals(env, i, obj->id);
    node = z_avl_iter_next(&iter);
  }

  node = z_avl_iter_seek_end(&iter);
  for (i = 8; node != NULL; i -= 2) {
    struct object_node *obj = z_container_of(node, struct object_node, link);
    z_assert_int_equals(env, i, obj->id);
    node = z_avl_iter_prev(&iter);
  }

  i = 3;
  node = z_avl_iter_seek_le(&iter, __object_key_cmp, &i, NULL);
  for (--i; node != NULL; i += 2) {
    struct object_node *obj = z_container_of(node, struct object_node, link);
    z_assert_int_equals(env, i, obj->id);
    node = z_avl_iter_next(&iter);
  ...
  i = 3;
  node = z_avl_iter_seek_ge(&iter, __object_key_cmp, &i, NULL);
  for (++i; node != NULL; i += 2) {
    struct object_node *obj = z_container_of(node, struct object_node, link);
    z_assert_int_equals(env, i, obj->id);
    node = z_avl_iter_next(&iter);
