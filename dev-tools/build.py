...
def runTool(tool, verbose=True):
  ...
  if exit_code != 0:
    ...
    tool_output.append(' [ OK ] %s %.2fsec' % (tool, elapsed))
...
class Project(object):
  ...
  def __init__(self, options):
    ...
    DEFAULT_LDLIBS = ['-lpthread', '-lm', '-lrt']
    ...
    #default_opts.addCFlags(['-Wmisleading-indentation'])
...
class R5LCore(Project):
  ...
  def setup_library(self):
    ...
    copy_dirs = [
                 ('devices', ['src/r5l-core/devices']),
                 ('objects', ['src/r5l-core/objects']),
                 #('space', ['src/r5l-core/space']),
                 #('key', ['src/r5l-core/plugins/key']),
                 #('oid', ['src/r5l-core/plugins/oid']),
                 #('semantics', ['src/r5l-core/semantics']),
                 #('format', ['src/r5l-core/plugins/format']),
                ]
  ...
  def build_tests(self):
    ...
    build = BuildMiniTools('r5l-test', ['tests/r5l-core'], options=build_opts)
    return build.build()
...
class R5LServer(Project):
  ...
  def build_tools(self):
    ...
    build_opts.addLdLibs(Zcl.get_ldlibs())
    build_opts.addIncludePaths(Zcl.get_includes())
    #build_opts.addLdLibs(R5LCore.get_ldlibs())
    #build_opts.addIncludePaths(R5LCore.get_includes())
...
def main(options):
  ...
  if not options.no_r5l:
    dependencies.extend([R5LCore, R5LServer, R5LClient])
